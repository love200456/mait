...
function insertChe(){
	...
	$.ajax({
		type : "post",
		url : '${basePath }ShopRecommendation/insertOneSR.action',
		data : {
			"sId" : checkedId,
			"sp_level" : "1"
		},
		...
		success : function(map) {
			...
			window.location.href = "${basePath }StoreBackstage/selectAllStoreXian.action?level=1"
		}
	});
}
</script>
...
<body>
	...
	$('.huoqu', parent.document).click(function(){
		...
		if(aa==2){
			...
			window.location.href = "${basePath }StoreBackstage/selectLikeLevelXian.action?searchVal="+cc+"&levels=1";
		}else{
			alert("店铺名称输入有误.....");
		}
	})
